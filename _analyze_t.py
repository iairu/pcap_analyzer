    ...
    def output(self, out):
        if (not self.unsupported):
            if (self.protocol_str == "TCP"):
                out["src_port"] = self.port_src
                out["dst_port"] = self.port_dst
                src_port_name = self.str_tcp(self.port_src)
                dst_port_name = self.str_tcp(self.port_dst)
                if (dst_port_name != "Unknown"):
                    out["app_protocol"] = dst_port_name
                elif (src_port_name != "Unknown"):
                    out["app_protocol"] = src_port_name

            elif (self.protocol_str == "UDP"):
                out["src_port"] = self.port_src
                out["dst_port"] = self.port_dst
                src_port_name = self.str_udp(self.port_src)
                dst_port_name = self.str_udp(self.port_dst)
                if (dst_port_name != "Unknown"):
                    out["app_protocol"] = dst_port_name
                elif (src_port_name != "Unknown"):
                    out["app_protocol"] = src_port_name
